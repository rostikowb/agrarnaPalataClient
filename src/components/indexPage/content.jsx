...
class Content extends Component {
    ...
    renderPosts(res) {
        try {
            ...
            this.setState({
                posts: res.reverse().map((key) => (
                        <React.Suspense
                            fallback={<div className="suspense"><img src='/assets/img/loader.gif' alt="loader"/></div>}>
                            <RecipeReviewCard
                                title={key.title}
                                imgSrc={key.content.match(/.+ src="(.+)" alt/)[1]}
                                content={key.short}
                                date={moment(+key.date).format('LL')}
                                id={key._id}
                            />
                        </React.Suspense>
                    )
